...
const AccountManager = () => {
  ...
  const handleLaunchGame = async () => {
    ...
    try {
      ...
      const loginCoords = await loadCoordinates(expansionData.coords_file);
  ...
  const handleChangeGamePath = async () => {
    try {
      ...
      if (!result.canceled && result.filePaths && result.filePaths[0]) {
        ...
        // Here you would normally update the server configuration
        // This is a simplified example - you need to implement the actual update logic
        
        // Example of how you might update the server configuration
        const updatedServers = {...servers};
        if (updatedServers[serverName] && 
            updatedServers[serverName].expansions && 
            updatedServers[serverName].expansions[expansionName]) {
          updatedServers[serverName].expansions[expansionName].path = newPath;
          // You would then need to save this using your context
          // For example: await updateServers(updatedServers);
        }
        
        setStatusMessage(`Game path updated to: ${newPath}`);
      }
    } catch (error) {
      console.error('Error selecting file:', error);
      setStatusMessage('Error updating game path');
    }
  };
      ...
      <header className="p-4 bg-gray-900 bg-opacity-80">
        ...
        <div className="flex justify-center mt-4 space-x-4">
          ...
          <div className="relative group">
            ...
            <div className="absolute hidden group-hover:block hover:block top-full left-0 mt-1 bg-gray-800 border border-gray-700 rounded shadow-lg z-10 w-48">
              <button 
